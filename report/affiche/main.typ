...
Le jeu pédagogique *"Shana a disparu"*, développé par le pôle Y-Security de la HEIG-VD, a rencontré un grand succès auprès du public en proposant une initiation au ethical hacking grâce à une histoire immersive. Cependant, il devient nécessaire de développer un nouveau jeu proposant des défis techniques plus avancés tout en maintenant l'accessibilité aux débutants. 
...
L'implémentation s'appuie sur l'architecture existante de la plateforme CyberGame avec un frontend développé en HTML, CSS et JavaScript, un backend Node.js avec Express, et des bases de données MongoDB et MySQL orchestrées par Docker. Pour enrichir l'expérience pédagogique, un IDE Python embarqué y est intégré permettant d'exécuter du code directement dans le navigateur, des terminaux SSH interactifs pour accéder à des environnements isolés, et un système de bot Puppeteer automatisé simulant des interactions réalistes. La validation des réponses repose sur un système sécurisé utilisant des hash SHA-3 256 stockés côté serveur. Des tests unitaires réalisés avec Jest ainsi que des tests utilisateurs aux profils variés ont permis d’identifier les points forts et les aspects à améliorer.
