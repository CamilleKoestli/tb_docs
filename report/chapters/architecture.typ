...
La plateforme _CyberGame_ est hébergée sur un serveur web accessible via le nom de domaine `heig-vd.ch` avec le sous-domaine `shana`. L'architecture repose sur une infrastructure conteneurisée orchestrée par Docker Compose, permettant une séparation claire des responsabilités entre les différents composants du système.
...
L'architecture adopte délibérément deux systèmes de gestion de base de données distincts pour séparer les responsabilités et les niveaux de sécurité. MongoDB stocke les données critiques de la plateforme : les flags hachés en SHA3-256 initialisés depuis les variables d'environnement `CHALL_FLAGS_2020`, `CHALL_FLAGS_2021` et `CHALL_FLAGS_2025`, les profils utilisateur avec les champs `uuid`, `name`, `surname`, `mail` et un tableau `flagged` pour suivre la progression, ainsi que les compteurs de visiteurs par heure. Cette base assure la persistance fiable des données utilisateur et du système de score. La base MySQL, initialisée via le script `init.sql` (Annexe@init.sql), sert exclusivement aux défis d'injection SQL. Elle contient deux tables : `users` avec les champs `ID` et `pass` (mots de passe volontairement stockés en clair), et `posts` pour les fonctionnalités de recherche. Cette base est intentionnellement dépourvue de protections (pas d'index, pas de contraintes d'intégrité) pour servir de cible d'apprentissage. L'objectif est d'isoler cette base de données vulnérable sans risquer d'altérer les vrais enregistrements MongoDB si un étudiant·e pousse l'exploit plus loin.

=== Routage des requêtes <routage-requetes>

Traefik v2.10 agit comme reverse proxy et point d'entrée unique pour tous les services. Il termine automatiquement les connexions HTTPS et applique des middlewares de type StripPrefix pour traiter les requêtes entrantes. Du côté client, le frontend peut appeler directement les endpoints via des préfixes standardisés comme `/backend/...` ou `/ssh/...` sans se préoccuper de la complexité du routage interne.

Le processus de transformation s'effectue ainsi : Traefik reçoit les requêtes avec préfixes, retire automatiquement ces préfixes via les middlewares configurés, puis transmet les requêtes aux services qui reçoivent des chemins sans préfixes. Par exemple, une requête `POST /backend/visitor` devient `POST /visitor` pour le service Express. De même, une requête `GET /ssh/?hostname=sshmachine` devient `GET /?hostname=sshmachine` pour le service wssh, qui se connecte ensuite en interne à `sshmachine:22`.
...
La page d'accueil index.html effectue un appel `POST /backend/visitor` pour incrémenter le compteur de visiteurs de la plateforme. La page de connexion `login.html` utilise deux endpoints : `POST /backend/login` pour l'authentification des utilisateurs et `GET /backend/logout` pour la déconnexion. La page de statistiques statistics.html interroge plusieurs endpoints via `GET /backend/stats/*` pour récupérer les données de progression et de performance.
...
L'API Express (détaillée en Annexe@index.js) constitue la couche serveur principale de la plateforme _CyberGame_. Elle est structurée autour de plusieurs modules fonctionnels qui gèrent les différents aspects du système de jeu et d'administration.
