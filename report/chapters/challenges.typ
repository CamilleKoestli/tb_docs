...
Après avoir retrouvé l'email de phishing à l'origine de l'attaque (#link(<ch-1>)[Challenge 1 Mail Contagieux]), il découvre le domaine frauduleux et se lance dans son exploration. Il y découvre un portail sûrement d'exfiltration mise en place par les assaillants (#link(<ch-2>)[Challenge 2 Portail Frauduleux]). Pour réussir à s'y introduire, il réalise une injection SQL pour contourner l'authentification et ouvrir une première session, mais seulement avec des droits utilisateur.
