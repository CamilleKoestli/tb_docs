...
*Outils nécessaires* : navigateur + DevTools, outil d’archive.
...
- XORer le début du chiffré avec CFG= ⇒ retrouver la clé `0x56 0x32 0x19 0x7A 0x56 0x32`.
...
*Indices graduels*
- Cherche la machine `ics-power-ctrl` – elle concentre les commandes critiques. 
- Filtre les requêtes `POST /launch` : ce sont des déclencheurs. 
- L’ID que tu dois envoyer est dans le champ `orchestration_id`. 
...
*Outils nécessaires*: binwalk / steghide / zsteg et un éditeur de texte pour ouvrir kill.txt

*Indices graduels*
- Le PNG est vraiment trop lourd ; qu’est-ce qui se cache après l’entête ?
- `binwalk` sait extraire les fichiers concaténés à une image. 
- Le mot de passe du défi 4 ouvre ce qui est planqué. 
